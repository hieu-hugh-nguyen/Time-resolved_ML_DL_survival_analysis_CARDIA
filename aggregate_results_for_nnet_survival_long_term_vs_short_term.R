...
for (fold in c(1,3:6,8:25)){  
  ...
  eligible_id = intersect(trainingid, data_after_y10_origin_at_10$ID)
  trained_data = data_after_y10_origin_at_10[which(data_after_y10_origin_at_10$ID %in% eligible_id),]
  ...
  loading.dir = paste0(work_dir, '/csv_files/nnet_survival/all_features/long_term')
  ...
  sub.dir = paste0('nnet_survival_all_var_long_term_fold_',fold)
...
for (fold in 2:25){
  ...
  trained_data = data_y10[which(data_y10$ID %in% eligible_id),]
  trained_data = within(trained_data, rm('ID'))
  test_data = data_y10[(which(!(data_y10$ID %in% eligible_id))),]
  test_data = within(test_data, rm('ID'))
  
  
  loading.dir = paste0(work_dir, '/csv_files/nnet_survival/all_features/short_term')
  pred_prob_surv = read.csv(file = paste0(loading.dir,
                                          '/pred_prob_surv_fold_',fold,'.csv'))
  pred_prob_surv = as.matrix(pred_prob_surv)
  prob_risk_test = 1-pred_prob_surv
  ...
  #saving.dir = 'U:/Hieu/CARDIA_project/CARDIA_project/Git/csv_files'
  main.dir = paste0(work_dir, '/rdata_files')
  sub.dir = paste0('nnet_survival_all_var_short_term_fold_',fold)
  if(!dir.exists(file.path(main.dir, sub.dir))){
    createDir(main.dir, sub.dir)
  }
  saving.dir = file.path(main.dir, sub.dir)
